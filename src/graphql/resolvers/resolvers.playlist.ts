...
import { fetchMediaItems } from "../repos/repos.media";
...
export async function showPlaylist(dynamicPlaylistConfig: DynamicPlaylistConfig) {
    const mediaItems = await fetchMediaItems();
    const filteredMediaItems = await fetchPlaylistPreview(mediaItems, dynamicPlaylistConfig);
